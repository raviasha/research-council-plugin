...
5. If Intent returns `awaiting_approval`, show its Intent Summary, Detailed Requirements, and draft to the user, then ask: **“Do you approve this brief and authorize research to begin?”** Pause the workflow. Resume Intent only after the user explicitly approves or provides corrections. Do not launch Researcher, Devil, Reviewer, or Documentation until Intent returns `approved` with a populated brief.
